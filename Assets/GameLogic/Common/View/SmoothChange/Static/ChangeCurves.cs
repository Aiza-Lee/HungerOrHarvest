...
namespace GameLogic.Common.View {
	public static class ChangeCurves {
		private static readonly Dictionary<ChangeCurveType, Func<float, float>> _curves;

		public static Func<float, float> GetCurve(ChangeCurveType type) {
			if (_curves.TryGetValue(type, out var curve)) {
				return curve;
			}
			throw new ArgumentException($"ChangeCurveType {type} is not defined.");
		}

		static ChangeCurves() {
			_curves = new Dictionary<ChangeCurveType, Func<float, float>> {
