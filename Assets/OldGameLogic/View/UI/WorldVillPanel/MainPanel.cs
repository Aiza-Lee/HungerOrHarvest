				...
				switch (clickedGroupType) {
					case GroupType.Arch: {
							DoCardsTransfer(tag.RectTrans, (vc) => {
								WorldMgr.Inst.FindWorkForVill(vc.AttachedVillID, (ArchType)clickedArchType, out var workArch);
								CmdRunner.Run($"/vill-bond-arch {vc.AttachedVillID} {workArch.ID}");
							});
							break;
						}
					case GroupType.Workless: {
							DoCardsTransfer(tag.RectTrans, (vc) => CmdRunner.Run($"/vill-disbond-workarch {vc.AttachedVillID}"));
							break;
						}
					case GroupType.Homeless: {
							DoCardsTransfer(tag.RectTrans, (vc) => CmdRunner.Run($"/vill-disbond-home {vc.AttachedVillID}"));
							break;
						}
					case GroupType.Home: {
							DoCardsTransfer(tag.RectTrans, (vc) => {
								WorldMgr.Inst.FindHomeForVill(vc.AttachedVillID, out var home);
								CmdRunner.Run($"/vill-bond-arch {vc.AttachedVillID} {home.ID}");
							});
							break;
						}
				}
